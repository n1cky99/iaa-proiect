...
class agent:
    '''
    Agenții pot fi plasați pe labirint.
    Ele pot reprezenta obiectul virtual doar pentru a indica celula selectată în Maze.
    Sau pot fi agenții fizici (cum ar fi roboții)
    Ele pot avea două forme (pătrat sau săgeată)
    '''
    def __init__(self,parentMaze,x=None,y=None,shape='square',goal=None,filled=False,footprints=False,color:COLOR=COLOR.blue):
        '''
        parentmaze-->  Labirintul pe care este plasat agentul.
        x,y-->  Poziția agentului, adică celula în interiorul căreia va fi plasat agentul
                Valoarea implicită este colțul din dreapta jos al Labirintului
        shape-->    pătrat sau săgeată (ca șir)
        goal-->     Valoarea implicită este scopul Labirintului
        filled-->   Pentru forma pătrată, filled=False este un pătrat mai mic
                    While filled=True este un pătrat mai mare umplut în celula completă
                    Această opțiune nu contează pentru forma săgeții.
        footprints-->   Când agentul se va muta într-o altă celulă, amprentele sale
                        pe celula anterioară poate fi plasat făcând acest lucru True
        color-->    Culoarea agentului.
        '''
        ...
        if(isinstance(color,str)):
            if(color in COLOR.__members__):
                ...
                raise ValueError(f'{color} nu este o CULOARE VALIDA!')
        self.filled=filled
        self.shape=shape
        self._orient=0
    ...
    def _RCCW(self):
        '''
        Pentru a roti agentul în direcția contra ceasului
        '''
    ...
    def _RCW(self):
        '''
        Pentru a roti agentul în direcția ceasului
        '''
        def pointNew(p,newOrigin):
            return (p[0]-newOrigin[0],p[1]-newOrigin[1])
        w=self._parentMaze._cell_width
...
class textLabel:
    '''
    Această clasă este de a crea Text Label pentru a afișa rezultate diferite în fereastră.
    '''
    def __init__(self,parentMaze,title,value):
        '''
        parentmaze-->   Labirintul pe care va fi afișată Eticheta.
        title-->        Titlul valorii care va fi afișată
        value-->        Valoarea care trebuie afișată
        '''
        self.title=title
        self._value=value
        self._parentMaze=parentMaze
...
class maze:
    '''
    Aceasta este clasa principală pentru a crea labirint.
    '''
    def __init__(self,rows=10,cols=10):
        '''
        rows--> Nr. de randuri ale labirintului
        cols--> No. de coloane ale labirintului
        Trebuie să treci doar cele două argumente. Restul va fi atribuit automat
        maze_map--> Va fi setat la un dicționar. Cheile vor fi celule și
                    valorile vor fi un alt dicționar cu chei=['E','W','N','S'] pentru
                    Est Vest Nord Sud și valorile vor fi 0 sau 1. 0 înseamnă că
                    direcția (EWNS) este blocată. 1 înseamnă că direcția este deschisă.
        grid--> O listă cu toate celulele
        path--> Cea mai scurtă cale de la început (dreapta jos) până la obiectiv (în mod implicit, stânga sus)
                Va fi un dicționar
        _win,_cell_width,_canvas --> _win și )canvas sunt pentru fereastra și pânza Tkinter
                                        _cell_width este lățimea celulei calculată automat
        _agens--> O listă de aganeți din labirint
        markCells--> Va fi folosit pentru a marca o anumită celulă în timpul
                        trasarii traseului de către agent.
        _
        '''
        self.rows=rows
        self.cols=cols
    ...
    def _Open_East(self,x, y):
        '''
        Pentru a îndepărta peretele de est al celulei
        '''
    ...
    def CreateMaze(self,x=1,y=1,pattern=None,loopPercent=0,saveMaze=False,loadMaze=None,theme:COLOR=COLOR.dark):
        ...
        pattern-->  Poate fi „v” pentru verticală sau „h” pentru orizontală
                    doar aspectul vizual al labirintului va fi mai vertical/orizontal
                    pasaje vor fi acolo.
        loopPercent-->  0 înseamnă că va exista o singură cale de la început până la obiectiv (labirint perfect)
                        O valoare mai mare înseamnă că vor exista mai multe căi (bucle)
                        Cu cât valoarea (maxim 100) este mai mare, buclele vor fi mai mari
        '''
        _stack=[]
        _closed=[]
        self.theme=theme
        ...
        if(isinstance(theme,str)):
            if(theme in COLOR.__members__):
                ...
                raise ValueError(f'{theme} nu este o temă de CULOARE VALIDA!')
        ...
        def removeWallinBetween(cell1,cell2):
            '''
            Pentru a îndepărta peretele dintre două celule
            '''
            if cell1[0]==cell2[0]:
        ...
        def isCyclic(cell1,cell2):
            '''
            Pentru a evita prea multă cale goală (cleară).
            '''
            ans=False
            if cell1[0]==cell2[0]:
        ...
        def BFS(cell):
            ...
            Pentru a genera calea cea mai scurtă.
            Acesta va fi folosit numai atunci când există mai multe căi (loopPercent>0) sau
            Maze este încărcat dintr-un fișier CSV.
            Dacă se generează un labirint perfect și fără fișierul de încărcare, această metodă 
            nu poate fi folosită deoarece generația Maze va calcula calea.
            '''
            frontier = deque()
            ...
            while cell!=(self.rows,self.cols):
                try:
                    ...
                    print('Calea către obiectiv nu a fost găsită!')
                    return
            return fwdPath
    ...
    def _drawMaze(self,theme):
        '''
        Crearea ferestrelor Tkinter și a liniilor de labirint
        '''
        
        self._LabWidth=26 # Space from the top for Labels
        self._win=Tk()
        self._win.state('zoomed')
        self._win.title('Maze Solver')
    ...
    def _redrawCell(self,x,y,theme):
        '''
        Pentru a redesena o celulă.
        Cu agent pătrat de dimensiune completă, se poate suprapune cu linii de labirint
        Deci celula este redesenată astfel încât liniile celulare să fie deasupra
        '''
        w=self._cell_width
        cell=(x,y)
        x=x*w-w+self._LabWidth
    ...
    def enableArrowKey(self,a):
        '''
        Pentru a controla un agent a cu tastele săgeți
        '''
    ...
    def enableWASD(self,a):
        '''
        Pentru a controla un agent a cu tastele W,A,S,D
        '''
    ...
    def _tracePathSingle(self,a,p,kill,showMarked,delay):
        '''
        O metodă internă pentru a ajuta metoda tracePath pentru urmărirea unei căi de către agent.
        '''
        
        def killAgent(a):
            '''
            dacă agentul ar trebui să fie ucis după ce atinge Scopul sau încheie calea
            '''
    ...
    def tracePath(self,d,kill=False,delay=300,showMarked=False):
        '''
        O metodă de a urmări calea după agent
        Puteți furniza mai mult de un agent/cale detalii
        '''
    ...
    def run(self):
        '''
        În cele din urmă pentru a rula Tkinter Main Loop
        '''
        self._win.mainloop()